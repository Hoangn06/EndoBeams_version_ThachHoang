...
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------
# STRESS AND STRAIN DATA STORAGE AND WRITING
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------
# INTERNAL FORCE AND MOMENT DATA STORAGE AND WRITING
# ------------------------------------------------------------------------------------------------
# ------------------------------------------------------------------------------------------------

# Structure to store internal force and moment data during simulation for all nodes
mutable struct InternalForceMomentStorage
    times::Vector{Float64}
    force_moments::Vector{Vector{Float64}}  # Each entry contains [Fx1, Fy1, Fz1, Mx1, My1, Mz1, Fx2, ...]
    n_nodes::Int  # Number of nodes (set on first accumulation)

    function InternalForceMomentStorage()
        new(Float64[], Vector{Float64}[], 0)
    end
end

# Function to accumulate internal force and moment data when simulation converges
function accumulate_internal_force_moment_data!(storage::InternalForceMomentStorage, conf::BeamsConfiguration, state::SimulationState, t::Float64)
    n_nodes = length(conf.nodes)
    if n_nodes >= 1
        # Set number of nodes on first call
        if storage.n_nodes == 0
        ...
        all_force_moments = Float64[]
        for node_idx in 1:n_nodes
            force_dofs = conf.nodes.global_dofs_disp[node_idx]
            moment_dofs = conf.nodes.global_dofs_rot[node_idx]

            Fint = state.forcesⁿ⁺¹.Tⁱⁿᵗ[force_dofs]
            Mint = state.forcesⁿ⁺¹.Tⁱⁿᵗ[moment_dofs]

            push!(all_force_moments, Fint[1])
            push!(all_force_moments, Fint[2])
            push!(all_force_moments, Fint[3])
            push!(all_force_moments, Mint[1])
            push!(all_force_moments, Mint[2])
            push!(all_force_moments, Mint[3])
        end
        push!(storage.force_moments, all_force_moments)
    end
end

# Function to write internal force and moment data to CSV file (Excel-compatible)
function write_internal_force_moment_data(storage::InternalForceMomentStorage, output_file::String="internal_force_moment_data.csv")
    n_timesteps = length(storage.times)
    n_nodes = storage.n_nodes
    ...
    # Create data matrix: 1 time column + 6*n_nodes force/moment columns
    data = zeros(n_timesteps, 1 + 6 * n_nodes)

    for i in 1:n_timesteps
        data[i, 1] = storage.times[i]
        for j in 1:(6 * n_nodes)
            data[i, 1 + j] = storage.force_moments[i][j]
        end
    end
    ...
    open(output_file, "w") do io
        header = "Time"
        for node in 1:n_nodes
            header *= ",FintX$node,FintY$node,FintZ$node,MintX$node,MintY$node,MintZ$node"
        end
        println(io, header)
        # Write data
        writedlm(io, data, ',')
