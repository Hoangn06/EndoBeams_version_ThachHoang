function compute_DFT_SEP_contributions( 
            K̄ⁱⁿᵗ_outer, ū, Θ̅₁, Θ̅₂, B¹, B¹¹, B²¹, B¹², B²², B¹⁴, B²⁴, exact, Tₛ⁻¹Θ̅₁, Tₛ⁻¹Θ̅₂,
    ...
    # ============================================================
    # Compute the superelastic contributions of the outer layer
    # ============================================================
    ...
    activation = any(DFT_SEP_state.superelasticⁿ⁺¹.H_AS .== 1.0) || any(DFT_SEP_state.superelasticⁿ⁺¹.H_SA .== 1.0) || any(DFT_SEP_state.superelasticⁿ⁺¹.H_SS .== 1.0)

    # If there is activation, compute the superelastic internal force and stiffness matrix
    if activation
        ...
        for i in 1:nᴳ
            ...
            for j in eachindex(zr)
                ...
                for k in eachindex(zθ)
                    ...
                    # Compute the variables used for integration (trapezoidal in r and θ)
                    pref = (l₀/2)
                    wtot = ωᴳ[i]*wr[j]*wθ[k] * r

                    # Compute the internal force in this Gauss point 
                    T̄ⁱⁿᵗ_gauss = superelasticity_internal_force(DFT_SEP_state, id_G, Be, beam_ind)
                    T̄ⁱⁿᵗ_outer += (pref*wtot)*T̄ⁱⁿᵗ_gauss
                    ...
                    K̄ⁱⁿᵗ_gauss = superelasticity_matrix_tangent(Eᴬ, Eᴹ, Gᴬ, Gᴹ, εL, α, R_f_AS, R_f_SA,
                                                    DFT_SEP_state, id_G, Be, beam_ind)

                    K̄ⁱⁿᵗ_outer += (pref*wtot)*K̄ⁱⁿᵗ_gauss
        ...
        K̄ⁱⁿᵗū_outer, K̄ⁱⁿᵗΘ̅_outer, K̄ⁱⁿᵗΘ̅Θ̅_outer = K̄ⁱⁿᵗ_outer

        # Compute elastic forces: T̄ⁱⁿᵗ = K̄ⁱⁿᵗ * D̄
        T̄ⁱⁿᵗū_outer  = K̄ⁱⁿᵗū_outer  * ū
        T̄ⁱⁿᵗΘ̅₁_outer = K̄ⁱⁿᵗΘ̅_outer  * Θ̅₁ + K̄ⁱⁿᵗΘ̅Θ̅_outer * Θ̅₂
        T̄ⁱⁿᵗΘ̅₂_outer = K̄ⁱⁿᵗΘ̅Θ̅_outer * Θ̅₁ + K̄ⁱⁿᵗΘ̅_outer  * Θ̅₂

        K̄ⁱⁿᵗ_outer = vcat(
            hcat(K̄ⁱⁿᵗū_outer, zero_row, zero_row),
            hcat(zero_row', K̄ⁱⁿᵗΘ̅_outer, K̄ⁱⁿᵗΘ̅Θ̅_outer),
            hcat(zero_row', K̄ⁱⁿᵗΘ̅Θ̅_outer, K̄ⁱⁿᵗΘ̅_outer)
        )
    end
    ...
    # Initialize inner layer contributions
    ...
    for i in 1:nᴳ
        ...
        for j in 1:nˢ
            for k in 1:nˢ
                ...
                sigma, C_const, converged = plasticity_return_mapping(E_inner, G_inner, ν_inner, y0, K,
                    DFT_SEP_state,
                    id_G, x1, x2, x3, l₀,
                    ū, Θ̅₁, Θ̅₂, beam_ind)
