...
E_inner = 50000                         # Inner Young's modulus (MPa)
ν_inner = 0.3                              # Inner Poisson's ratio
ρ_inner = 6.5e-9                                # Inner Density (tonnes/mm³)
y0 = 170.0                               # Inner yield stress (MPa)
K = 500.0                               # Inner plastic hardening modulus (MPa)
radius_inner = 0.02                      # Inner Beam radius (mm)
...
beams = DFT_EPBeams(nodes, connectivity, E, ν, ρ, E_inner, ν_inner, ρ_inner, radius_inner, y0, K, radius, damping)
...
# DFT-EP has only inner plastic Gauss points in stress_strain_data
n_gp_expected = params.nᴳ_beams * (params.nˢ_beams * params.nˢ_beams)
if n_gp_expected != n_gauss_points
    @warn "Detected $n_gauss_points GP columns, expected $n_gp_expected for DFT-EP."
end

p_stress_strain = plot(
    xlabel="Von-Mises Strain",
    ylabel="Von-Mises Stress (MPa)",
    title="Von-Mises Stress vs. Von-Mises Strain (DFT-EP inner plastic)",
    titlefontsize=30,
    xguidefontsize=28,
    yguidefontsize=28,
    xtickfontsize=24,
...
for gp in 1:n_gauss_points
    ...
    plot!(p_stress_strain, strain_gp, stress_gp,
        ...
        linealpha=0.75,
        color=:firebrick,
        label=(gp == 1 ? "Inner plastic GPs ($n_gauss_points)" : "")
    )
end

# Add figure caption below xlabel (compact version)
plot!(p_stress_strain, 
    xlabel="Von-Mises Strain\n──────────────────────────────────────────────────────────── \n Fig.1. Von-Mises stress-strain for DFT-EP beam (inner plastic)")
