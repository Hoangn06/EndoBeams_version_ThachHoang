...
y0_outer = 170.0                  # Outer yield stress (MPa)
K_outer = 100.0                   # Outer plastic hardening modulus (MPa)
...
radius_inner = 0.03              # Inner radius (mm)
y0_inner = 600.0                  # Inner yield stress (MPa)
K_inner = 10000.0                   # Inner plastic hardening modulus (MPa)
...
beams = DFT_PPBeams(
    ...
    E_inner, ν_inner, ρ_inner, radius_inner, y0_inner, K_inner,
    radius, damping
)
...
# DFT split:
# - outer plastic points are stored first
...
p_stress_strain = plot(
    ...
    title="Von-Mises Stress vs. Von-Mises Strain (DFT-PP: Outer + Inner)",
    titlefontsize=30,
    xguidefontsize=28,
    yguidefontsize=28,
    xtickfontsize=24,
...
for gp in 1:n_gp_outer
    ...
    plot!(p_stress_strain, strain_gp, stress_gp,
        ...
        linealpha=0.65,
        color=:royalblue,
        label=(gp == 1 ? "Outer plastic GPs ($n_gp_outer)" : "")
    )
end
...
plot!(p_stress_strain, 
    xlabel="Von-Mises Strain\n──────────────────────────────────────────────────────────── \n Fig.1. Von-Mises stress-strain for DFT-PP beam (solid: outer plastic, dashed: inner plastic)")
