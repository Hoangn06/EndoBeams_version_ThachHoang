...
total_spring_height = 35.07  # Total free length of the spring (mm)
...
force_function(t,i) = t <= 30.0 ? 35.0 * t / 30.0 : 35.0 * (60.0 - t) / 30.0   # 0→30N (t=0→30s), 30→0N (t=30→60s)
...
forces = [t <= 30.0 ? 35.0 * t / 30.0 : 35.0 * (60.0 - t) / 30.0 for t in times]
